...
def train_loop(network, absolute_positions, nodes_features, edge_indexes, edges_features, latent_variables):
    ...
    all_losses = []
    latent_vars = latent_variables * 4*torch.randn((10,3)) #np.random.normal(scale=4, size=(1, 3), dtype=float)
    ...
    for epoch in range(1000):
        for i in range(1000):
            ...
            loss = network.loss(new_structure, absolute_positions + latent_var**2, mask_weights)
...
def experiment(graph_file="data/features.npy"):
    ...
    translation_mlp = MLP(3, 3, 100, num_hidden_layers=2)
