...
def train_loop(network, absolute_positions, renderer, local_frame, generate_dataset=True,
    ...
    if generate_dataset:
        ...
        conformation1_rotation_angle = torch.tensor(np.array([0, 0, 0, 0]), dtype=torch.float32)
        ...
        conformation2_rotation_angle = torch.tensor(np.array([0, 0, 0, 0]), dtype=torch.float32)
