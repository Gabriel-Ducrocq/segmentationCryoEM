...
def train_loop(network, absolute_positions, nodes_features, edge_indexes, edges_features, latent_variables):
    ...
    for epoch in range(1000):
        ...
        for i in range(1000):
            ...
            true_deformed_structure[:3*cutoff, :] = absolute_positions[:3*cutoff, :] + latent_var[:3]
            true_deformed_structure[3 * cutoff:, :] = absolute_positions[3 * cutoff:, :] + latent_var[3:]
            ...
            print("Latent var:", latent_var)
            print("Mean:", torch.mean(latent_vars, dim=0))
            print(loss)
            print("\n\n")
