...
class Net(torch.nn.Module):
    ...
    def sample_latent(self, distrib_parameters, images=None):
        ...
        if self.use_encoder:
            ...
            latent_mu_angle = latent_mu_angle/(torch.sqrt(torch.sum(latent_mu_angle**2, dim=-1))[:,:, None] + 1e-5)
            ...
            all_latent_angle_powSphForm = torch.zeros((self.batch_size, self.N_domains, 3), device=self.device)
