...
class Net(torch.nn.Module):
    def __init__(self, N_residues, N_domains, latent_dim, B, S, encoder, decoder, renderer, local_frame, atom_absolute_positions,
        ...
        self.latent_mean = torch.nn.Parameter(data=torch.randn((20000, self.latent_dim)), requires_grad=True)
        self.latent_std = torch.nn.Parameter(data=torch.randn((20000, self.latent_dim)), requires_grad=True)
        self.tau = 0.05
