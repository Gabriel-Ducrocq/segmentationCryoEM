...
class Net(torch.nn.Module):
    def __init__(self, N_residues, N_domains, latent_dim, B, S, encoder, decoder, renderer, local_frame, atom_absolute_positions,
        ...
        self.dim = 3
        ##Forces:
        self.mean_translations = torch.nn.Parameter(torch.zeros((N_domains, 3), dtype=torch.float32, device=device), requires_grad=True)
        self.std_translations = torch.nn.Parameter(torch.ones((N_domains, 3), dtype=torch.float32, device=device), requires_grad=True)

        self.mean_quaternions = torch.nn.Parameter(torch.zeros((N_domains, 3), dtype=torch.float32, device=device), requires_grad=True)
        self.std_quaternions = torch.nn.Parameter(torch.ones((N_domains, 3), dtype=torch.float32, device=device), requires_grad=True)
